...
| `migration_v45_kb_context_state.sql` | **6.4up · KB 会话级记忆池**【🔑 6.4up 必跑】新表 `kb_context_state(conversation_id, chunk_id, first_seen_at, last_hit_at, hit_count, last_similarity)` + 2 索引（conv+last_hit_at desc / conv+hit_count desc）+ 3 RPC：`bump_kb_context_state`（atomic upsert + hit_count++ + is_current 控制 last_similarity 写入）、`trim_kb_context_state`（按 last_hit_at asc 裁剪到 KB_MEMORY_POOL_SIZE）、`rank_kb_context_chunks`（池内 chunks 用本轮 query embedding 重算 cosine，与 v42 同 active KB + done 文档过滤口径）。ON DELETE CASCADE 保证 conversation / chunk 删除时池自动清理。⚠ 不跑此条 → 6.4up 代码层调 RPC 全部 404，整条 KB 链路降级失效。 | ☐ |
| `migration_v46_workflow_scope_order.sql` | **6.3up · 工作流分层级配置**【🔑 6.3up 工作流配置必跑】新表 `workflow_scope_order(scope_type ∈ org/dept/team, scope_id TEXT, workflow_id UUID, sort_order INT)` PK 复合；2 索引（workflow_id / scope+sort_order）+ 2 RPC：`get_user_workflow_order(user_id)`（DISTINCT ON 层级回退 team>dept>org 取最具体排序）、`batch_reorder_workflow_scope(scope_type, scope_id, ordered_ids[])`（事务化重排 = DELETE + INSERT 避主键冲突）。与 `resource_permissions` 配对：管理员"拉取"工作流到层级 = 同步写两张表（visibility + order 解耦但 UI 入口一体）。⚠ 不跑此条 → `/admin/workflow-config` 配置页 + `/api/admin/workflow-scope-order` CRUD + `/api/workflows` 层级排序回退全部 5xx。 | ☐ |
| `migration_v47_workflow_scope_order_rpc.sql` | **6.3up R1.2 Finding 4** 【🔑 6.3up 必跑 · 配 v46 一起】新增 2 RPC：`add_workflow_scope_order(scope_type, scope_id, workflow_ids[])`（CTE 事务化同步写 workflow_scope_order + resource_permissions，sort_order 接末尾，已存在跳过）、`remove_workflow_scope_order(scope_type, scope_id, workflow_id)`（事务化同步删两张表）。修复 v46 route 层 POST/DELETE 分两步写两表导致的"半成功"竞态（中间失败留下"有排序无可见"或"可见无排序"）。⚠ 不跑此条 → `/api/admin/workflow-scope-order` POST/DELETE 报"function does not exist"。 | ☐ |
| `migration_v48_perms_to_scope_order_trigger.sql` | **6.3up R1.9** 【🔑 6.3up 必跑 · 配 v46+v47 一起】新增 trigger function `sync_workflow_scope_order_from_perms()` + trigger `tr_resource_perms_sync_scope_order`（AFTER INSERT/DELETE ON resource_permissions）：workflow + org/dept/team scope 的 perms 写入/删除自动反向维护 `workflow_scope_order` 行（INSERT 接末尾 + ON CONFLICT DO NOTHING；DELETE 精确匹配）。含一次性 backfill：把已有 perms 中存在但 order 中缺失的工作流补齐，确保 trigger 生效前历史数据也一致。⚠ 不跑此条 → 工作流编辑器写「指定 X 可见」后该工作流不会在分层级配置页对应 scope 显示（UX 缝隙）。与 v47 RPC 协作：trigger 触发时 RPC 已先写 order，反向 INSERT 主键冲突 ON CONFLICT 跳过 / RPC 已先删 order，反向 DELETE 0 行，无副作用。 | ☐ |
| `migration_v49_workflows_created_by_drop_fk.sql` | **6.3up R1.10 · 新建工作流报"关联数据不存在" 修复** 【🔑 必跑】drop `workflows.created_by` 的 FK 约束（原 v31 `REFERENCES admins(id)`）。与 v28 修 `audit_logs.admin_id` 同款 bug：从 users 表提升为管理员的账号 adminId 来自 users.id，admins 表里没这 id，写 workflows 时 FK 23503。用 DO BLOCK 兜底任意约束名。created_by 字段本身保留，业务层按 created_by_role 判定上下级权限，不依赖 FK。⚠ 不跑此条 → 非内置 admin 创建工作流报"关联数据不存在"。 | ☐ |
| `migration_v50_phone_unique.sql` | **6.5up · 一个手机号锁一个有效用户** 【🔑 必跑 · 上线前要确认 prod 无 active+disabled 重复手机号】新增 partial unique index `users_phone_active_unique ON users (phone) WHERE status IN ('active', 'disabled')`。schema.sql:31 的复合 UNIQUE (phone, tenant_code) 保留（防同组织同号重复），v50 在其上加全局约束：跨组织 / user_type 也不能重复；deleted / cancelled 不占名额，原主人可后续重新注册。`/api/auth/register` 同步加跨组织全表预检查并返"该手机号已注册"。⚠ 上线前要先跑排查 SQL：`SELECT phone, COUNT(*) FROM users WHERE status IN ('active','disabled') GROUP BY phone HAVING COUNT(*) > 1;` 非空必须先清理（保留最早 / 最近登录的一条，其他改 status='deleted'）再跑 v50，否则 CREATE INDEX 报错。 | ✅ 2026-06-05（prod 排查 0 重复，免清理直接上） |
| `migration_v50_custom_roles.sql` | ⚠ **v50 撞号**：与上面 6.5up 的 `migration_v50_phone_unique.sql` 同号不同文件（两者内容独立、文件名不同可分别跑，但编号重复）。**合并后建议把本条重编为 v53** 以恢复单调递增；改名前先确认各环境是否已按 v50 跑过本文件。<br>**6.4up · 权限管理 · 自定义角色基建**【🔑 权限管理上线必跑】新建 3 张表 `custom_roles` / `custom_role_permissions` / `user_custom_roles`；`workflows` 加 `created_by_kind`（CHECK admin/custom_admin）+ `created_by_role_code`；历史行回填 `kind='admin', role_code=created_by_role`。旧 `created_by_role` 列与 v31 CHECK 不动。permission_key 不在 DB 加 CHECK（由 `lib/permission-keys.ts` + API 校验）。⚠ 不跑此条 → 自定义角色 CRUD / 用户绑角色 / custom admin 创建工作流全 500。 | ☐ |
...
| `migration_v52_permission_v2.sql` | **6.4up v2 Phase A / 6.6up seed refresh · 权限管理 v2 基建**【🔑 v2 enforce 上线必跑】新建 2 张表 `builtin_role_permissions`（system_admin / org_admin 默认包，CHECK 不含 super_admin）+ `admin_permission_overrides`（个人 grant/revoke，PK 含 admin_source 防 admin_table vs user_admin UUID 撞）；seed 185 条（system_admin 117 / org_admin 68，含 6.6up `user_group.*.{org,all}`）；新建 RPC `change_user_role_clear_custom(p_user_id, p_new_role, p_actor_id)`。**6.6up 行为切换**：`PERMISSION_V2_ENFORCE_RESOURCES` 未设置/`all` = 全资源 enforce；`none` = 紧急回滚；CSV = 灰度指定资源。⚠ 不跑此条 → v2 默认包/个人 override 缺表，后台权限 enforce 路径 500。 | ☐ |
...
| `migration_v57_agent_effective_admin_scopes.sql` | **7.23up · Agent Effective Scope**【🔑 Dashboard Summary V2 / Agent Center V2 上线门禁】新增 `security_invoker` 视图，统一显式 `all/org/dept/team` scope 与“完全无任何权限行时”的发布草稿创建者回退；创建者组织使用 `COALESCE(admin.tenant_code, user.tenant_code)`。撤销 `PUBLIC/anon/authenticated` 查询权，仅授权 `service_role`。⚠ 未执行或权限测试未通过时，组织范围 `totalAgents` 必须返回 `null`，Agent Center V2 不得开启。 | ☐ |
| `migration_v58_admin_analytics_rpc.sql` | **7.23up · Analytics / Dashboard SQL 聚合**【🔑 Analytics V2 / Dashboard Summary V2 必跑】新增 `admin_usage_summary`、`admin_user_usage_page`、`admin_visible_agent_count`，全部 `SECURITY INVOKER`；Analytics 按 `(tenant_code,user_phone)` 聚合并 LEFT JOIN 当前用户，Top 智能体只按 `agent_code` 分组；组织范围智能体计数通过 Effective Scope 的“无越界 scope”条件计算。全部撤销 `PUBLIC/anon/authenticated` 执行权，仅授权 `service_role`。 | ☐ |
| `migration_v59_admin_agent_center_rpc.sql` | **7.23up · Agent Center SQL 分页**【🔑 Agent Center V2 必跑】新增 `admin_agent_center_page`，直接关联 v57 Effective Scope，在数据库内按“无任何越界 scope”的 AND 语义完成可见性、筛选、total、状态/分类统计和分页；`.all` 是唯一范围绕过路径，dept/team 反查失败 fail-closed。引用数、会话数和当前页引用名称均在 SQL 聚合。`SECURITY INVOKER` 且仅授权 `service_role`。 | ☐ |
| `ops/7.23up_agent_center_indexes.sql` | **7.23up · Agent Center 并发索引运维脚本**【v59 放量前门禁】使用独立、非事务 `CREATE INDEX CONCURRENTLY` 创建 `conversations/workflow_steps/agent_knowledge_bases(agent_id)` 三个索引。执行前后必须保存 `ops/7.23up_explain.sql` 计划，并运行 `npm run verify:7.23up:db`。 | ☐ |
