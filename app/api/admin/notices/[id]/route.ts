...
export async function PATCH(
  ...
  const accessErr = await requireAccess(ctx.actor, "notice", "update", {
  ...
  // org_admin / custom_admin 不允许修改 tenantCode（防止改成全局公告）
  if (body.tenantCode !== undefined && ctx.role !== "org_admin" && !ctx.isCustomAdmin) {
...
export async function DELETE(
  ...
  const accessErr = await requireAccess(ctx.actor, "notice", "delete", {
    row: { id, tenant_code: (noticeRow as { tenant_code: string | null }).tenant_code },
  });
  if (accessErr) return accessErr;
