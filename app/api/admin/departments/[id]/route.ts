...
export async function PATCH(
  ...
  // Phase D D-4 · v2 第二闸（env-gated；dept.update）
  if (isResourceEnforced("dept") && admin.role !== "super_admin") {
    const actor = await buildPermissionActor(admin);
    const err = await requireAccess(actor, "dept", "update", { id });
    if (err) return err;
  }
...
export async function DELETE(
  ...
  // Phase D D-4 · v2 第二闸（env-gated；dept.delete）
  if (isResourceEnforced("dept") && admin.role !== "super_admin") {
    const actor = await buildPermissionActor(admin);
    const err = await requireAccess(actor, "dept", "delete", { id });
    if (err) return err;
  }
