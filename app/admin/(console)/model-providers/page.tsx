...
export default function ModelProvidersPage() {
  const { me: sessionMe } = useAdminSession();
  ...
  const meRole = sessionMe?.role ?? sessionMe?.builtinRole ?? null;
  const me: MeInfo | null = meRole
    ? { role: meRole, tenantCode: sessionMe?.tenantCode ?? null }
    : null;
  ...
  useEffect(() => {
    if (!meRole || meRole === "org_admin") return;
