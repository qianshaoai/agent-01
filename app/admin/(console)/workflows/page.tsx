...
export default function WorkflowsAdminPage() {
  ...
  const { me: adminMe } = useAdminSession();
  const adminRole = adminMe?.role ?? adminMe?.builtinRole ?? null;
  const adminTenantCode = adminMe?.tenantCode ?? null;
  const accessSource = adminMe?.source ?? null;
  const customPermissions = new Set(adminMe?.permissions ?? []);
