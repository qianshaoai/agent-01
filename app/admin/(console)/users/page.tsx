...
export default function AdminUsersPage() {
  ...
  const currentRole = me?.role ?? me?.builtinRole ?? null;
  const adminMeta: AdminMeta | null = currentRole
    ? { role: currentRole, tenantCode: me?.tenantCode ?? null }
    : null;
  ...
  async function openDeptModal(u: UserRow) {
    ...
    setTeamOptions([]);
    const orgTree = await getAdminOrgTree();
    setDeptOptions(orgTree.departments.filter((dept) => dept.tenant_code === u.tenant_code));
    // 如果已有部门，加载对应小组
    if (u.dept_id) {
      setTeamOptions(orgTree.teams.filter((team) => team.dept_id === u.dept_id));
    }
  }
  ...
  async function handleDeptChange(deptId: string) {
    ...
    if (!deptId) return;
    const orgTree = await getAdminOrgTree();
    setTeamOptions(orgTree.teams.filter((team) => team.dept_id === deptId));
  }
